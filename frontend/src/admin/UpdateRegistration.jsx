...
function UpdateRegistration() {
  ...
  // ✅ Fetch user by id and pre-fill form
  useEffect(() => {
    const fetchUser = async () => {
      try {
          ...
          headers: {
            ...
            auth: token  // ✅ token added
          },
        });
        const data = await res.json();
        if (data.success) {
          setFormData(data.user); // Prefill inputs with backend user data
        }
        setLoading(false);
      } catch (err) {
  ...
  return (
      ...
      <div className="bg-white shadow-lg rounded-2xl p-8 w-full max-w-2xl">
        ...
        <div className="flex justify-between mt-6">
          <button onClick={() => navigate("/admin/records")} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
            Cancel
          </button>
