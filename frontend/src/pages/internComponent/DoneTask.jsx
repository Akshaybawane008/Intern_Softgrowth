...
import { Search, FileText, Calendar, User, Eye, Download, CheckCircle, Award } from "lucide-react";
...
const IntenRecord = () => {
  ...
  // Filter completed tasks + apply search
    ...
    <div className="w-full max-w-7xl mx-auto p-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Great work! Here are all the tasks you've successfully completed
          </p>
        </div>
        
        {/* Stats Card */}
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4 mt-4 lg:mt-0">
          <div className="flex items-center gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600 dark:text-green-400">{filteredTasks.length}</div>
              <div className="text-sm text-green-600 dark:text-green-400">Completed</div>
            </div>
            <div className="h-8 w-px bg-green-200 dark:bg-green-700"></div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-600 dark:text-gray-400">
                {tasks.filter(t => t.statusbar === "completed").length}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400">Total Done</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search completed tasks by student name, task description, or remarks..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
          />
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-all duration-200 text-sm font-medium group/btn hover:shadow-lg"
                      >
                        <Eye size={16} />
                        View Details
                      </button>
...
<div className="block lg:hidden">
  {filteredTasks.length > 0 ? (
    filteredTasks.map((task, index) => (
      ...
      >
        ...
        <div className="mt-3">
          <button
            onClick={() => navigate(`/intern/task/${task._id}`)}
            className="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors duration-200 text-sm font-medium flex items-center justify-center gap-2"
          >
            <Eye size={16} />
            View Details
          </button>
    ...
    <div className="text-center py-12 text-gray-500 dark:text-gray-400">
      ...
      <h3 className="text-lg font-semibold mb-2">No completed tasks found</h3>
      <p>{searchTerm ? "No completed tasks match your search criteria" : "You haven't completed any tasks yet"}</p>
      {searchTerm && (
        <button
          onClick={() => setSearchTerm("")}
